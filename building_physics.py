...
# with binary control
    def solve_energy_with_binary_control(self, internal_gains, solar_gains, t_out, t_m_prev,
                                         heating_action, cooling_action,
                                         fixed_heating_power=None, fixed_cooling_power=None):
        """
        Calculates building energy with binary control actions (0 or 1)
        ...
        :param t_m_prev: Previous thermal mass temperature [C]
        :param heating_action: Binary control signal (0=off, 1=on)
        :param cooling_action: Binary control signal (0=off, 1=on)
        :param fixed_heating_power: Fixed heating power when ON [W], defaults to 10*floor_area
        :param fixed_cooling_power: Fixed cooling power when ON [W], defaults to -10*floor_area
        """

        # Set default fixed powers if not provided
        # if fixed_heating_power is None:
        #     fixed_heating_power = 30 * self.floor_area  # 60 W/m2  for hotter annual result
        # if fixed_cooling_power is None:
        #     fixed_cooling_power = -30 * self.floor_area  # -60 W/m2
        if fixed_heating_power is None:
            fixed_heating_power = 20 * self.floor_area  # 60 W/m2   # for less hotter
        if fixed_cooling_power is None:
            fixed_cooling_power = -20 * self.floor_area

        # Determine energy demand based on binary actions
        if heating_action == 1 and cooling_action == 0:
            self.energy_demand = fixed_heating_power
            self.has_heating_demand = True
            self.has_cooling_demand = False
        elif cooling_action == 1 and heating_action == 0:
            self.energy_demand = fixed_cooling_power
            self.has_heating_demand = False
            self.has_cooling_demand = True
        else:
            # Both off or invalid state
            self.energy_demand = 0
            self.has_heating_demand = False
            self.has_cooling_demand = False

        # Calculate temperatures with the determined energy demand
        self.calc_temperatures_crank_nicolson(
            self.energy_demand, internal_gains, solar_gains, t_out, t_m_prev)

        # Calculate supply system energy consumption
        if self.has_heating_demand or self.has_cooling_demand:
            self.calc_heat_flow(t_out, internal_gains, solar_gains, self.energy_demand)
